...
void sdes_cbc_encrypt(uint8_t *plaintext, uint8_t *ciphertext, uint8_t *key, uint8_t *iv, size_t length);
void sdes_cbc_decrypt(uint8_t *ciphertext, uint8_t *plaintext, uint8_t *key, uint8_t *iv, size_t length);

int main() {
    // Key and Initialization Vector (IV)
    uint8_t key[10] = {0, 1, 1, 1, 1, 1, 1, 0, 1, 1}; // Key: 01111 11101
    uint8_t iv[BLOCK_SIZE] = {1, 0, 1, 0, 1, 0, 1, 0}; // IV: 1010 1010

    // Test plaintext
    uint8_t plaintext[BLOCK_SIZE] = {0, 0, 0, 1, 0, 0, 1, 1}; // 0000 0001 0010 0011

    uint8_t ciphertext[BLOCK_SIZE];
    uint8_t decrypted[BLOCK_SIZE];

    // Encryption
    sdes_cbc_encrypt(plaintext, ciphertext, key, iv, BLOCK_SIZE);
    ...
    // Decryption
    sdes_cbc_decrypt(ciphertext, decrypted, key, iv, BLOCK_SIZE);
...
void sdes_cbc_encrypt(uint8_t *plaintext, uint8_t *ciphertext, uint8_t *key, uint8_t *iv, size_t length) {
    ...
    for (size_t i = 0; i < length; i += BLOCK_SIZE) {
        ...
        xor_blocks(block, previous, block);
...
void sdes_cbc_decrypt(uint8_t *ciphertext, uint8_t *plaintext, uint8_t *key, uint8_t *iv, size_t length) {
    ...
    for (size_t i = 0; i < length; i += BLOCK_SIZE) {
        uint8_t block[BLOCK_SIZE];
        sdes_decrypt(*ciphertext, key, block);

        xor_blocks(block, previous, plaintext + i);
